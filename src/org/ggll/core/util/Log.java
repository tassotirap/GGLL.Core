...
/**
 * This class provides methods for logging events. In terms of functionality, it
 * is somewhere in between <code>System.out.println()</code> and full-blown
 * logging packages such as log4j.
 * <p>
 * 
 * All events are logged to an in-memory buffer and optionally a stream, and
 * those with a high urgency (warnings and errors) are also printed to standard
 * output.
 * <p>
 * 
...
public class Log
{
	...
	/**
	 * A print stream that uses the "Log" class to output the messages, and has
	 * special treatment for the printf() function. Using this stream has one
	 * caveat: printing messages that don't have a line break at the end will
	...
	private static class LogPrintStream extends PrintStream
	{
		...
		// {{{ printf() method
		/**
		 * This is a hack to allow "printf" to not print weird stuff to the
		 * output. Since "printf" doesn't seem to print the whole message in one
		 * shot, our output stream above would break a line of log into several
		 * lines; so we buffer the result of the printf call and print the whole
		 * thing in one shot. A similar hack would be needed for the "other"
		 * printf method, but I'll settle for the common case only.
		 */
		@Override
	 ...
	 * 
	 * @param stdio
	 *            If true, standard output and error will be sent to the log
	 * @param level
	 *            Messages with this log level or higher will be printed to the
	 *            system console
	 * @since jEdit 3.2pre4
	 */
	 ...
	 * 
	 * @param urgency
	 *            The urgency; can be one of <code>Log.DEBUG</code>,
	 *            <code>Log.MESSAGE</code>, <code>Log.NOTICE</code>,
	 *            <code>Log.WARNING</code>, or <code>Log.ERROR</code>.
	 * @param source
	 *            The source of the message, either an object or a class
	 *            instance. When writing log messages from macros, set this
	 *            parameter to <code>BeanShell.class</code> to make macro errors
	 *            easier to spot in the activity log.
	 * @param message
	 *            The message. This can either be a string or an exception
	 * 
	 * @since jEdit 2.2pre2
	 */
	public synchronized static void log(int urgency, Object source, Object message)
	{
		String _source;
		if (source == null)
		{
	 ...
	 * 
	 * If an exception is the cause of a call to {@link #log}, then the
	 * exception should be explicitly provided so that it can be presented to
	 * the (debugging) user in a useful manner (not just the exception message,
	 * but also the exception stack trace)
	 * 
	 * @since jEdit 4.3pre5
	 */
